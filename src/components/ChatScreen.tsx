...
export const ChatScreen: React.FC<{
  ...
  useEffect(() => {
    const eventFuntion = async (event: KeyboardEvent) => {
      if (event.key === "Enter") {
        promptEnterButtonRef.current && promptEnterButtonRef.current.click();
      }
    };
    ...
    return promptInputRef.current?.removeEventListener("keypress", (event) =>
      eventFuntion(event)
    );
  }, []);
  /**
  ...
  return (
    ...
    >
      ...
      >
        <ChatScreeNavBar isOpen={isOpen} toggleSideBar={toggleSideBar} />
      </div>
      {/* Messages */}
      <div
      ...
      >
        {messages && !isGettingMessage ? (
          messages.map((message, index) => {
            return (
              <div
                ref={index === messages.length ? bottomMessageRef : null}
                key={message._id}
